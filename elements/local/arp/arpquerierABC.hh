#ifndef CLICK_ARPQUERIERABC_HH
#define CLICK_ARPQUERIERABC_HH
#include <click/element.hh>
#include <click/etheraddress.hh>
// #include <click/ipaddress.hh>
#include <click/sync.hh>
#include <click/timer.hh>
#include "arptableABC.hh"
...
ARPQuerierABC(IP, ETH, I<keywords>)
ARPQuerierABC(NAME, I<keywords>)
...
ARPQuerierABC may have one or two outputs. If it has two, then ARP queries
are sent to the second output.

ARPQuerierABC implements special behavior for 0.0.0.0, 255.255.255.255, multicast
...
Element.  Names an ARPTableABC element that holds this element's corresponding
ARP state.  By default ARPQuerierABC creates its own internal ARPTableABC and uses
...
Amount of time after which ARPQuerierABC will start polling for renewal.  0 means
...
=e
   ...
   a :: ARPQuerierABC(18.26.4.24, 00:00:C0:AE:67:EF);
   c[0] -> a[1];
   c[1] -> ... -> a[0];
   a[0] -> ... -> ToDevice(eth0);
...
instances of ARPQuerierABC.

ARPQuerierABC uses packets' destination IP address annotations, and can destroy
their next packet annotations.  Generated ARP queries have VLAN TCI
annotations set from the corresponding input packets.

ARPQuerierABC will send at most 10 queries a second for any IP address.

=h ipaddr rw

Returns or sets the ARPQuerierABC's source IP address.

=h broadcast r

Returns the ARPQuerierABC's IP broadcast address.

=h table r

Returns a textual representation of the ARP table.  See ARPTableABC's table
handler.
...
ARPTableABC, ARPResponder, ARPFaker, AddressInfo
*/

class ARPQuerierABC : public Element { public:

    ARPQuerierABC();
    ~ARPQuerierABC();

    const char *class_name() const		{ return "ARPQuerierABC"; }
